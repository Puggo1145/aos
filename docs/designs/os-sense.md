...
// `VisualMirror` 不是 SenseContext 的字段。视觉兜底是 submit-time on-demand
// 截图（见 §"ScreenMirror（视觉兜底）"），由 Shell 在用户提交时调用
// `SenseStore.captureVisualSnapshot()` 单帧捕获，传给 CitedContext 投影。
...
- UI 显示 chip 的条件：`SenseStore.visualSnapshotAvailable` —— `app != nil && !permissions.denied.contains(.screenRecording)`。Notch UI 还会再叠一层 `behaviors.isEmpty` 来决定是否露出 chip（有结构化 behaviors 时不需要兜底）
- 真正捕获的条件：用户保持 chip 选中并点 send。`AgentInputField.submit()` 调用 `await senseStore.captureVisualSnapshot()` → Shell 把返回的 `VisualMirror?` 作为参数传给 `CitedContextProjection.project(from:selection:visual:)`
- app 切换 / chip 取消 / behaviors 变非空 / 提交未点 send：什么都不发生，因为根本没有捕获在跑
